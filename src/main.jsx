...
import { sanitizeGames } from './sports/data-reliability.js';
import { getGameStateRefreshDelay, mergeLiveGames } from './sports/live-state.js';
import { toGameViewModels } from './sports/ui/game-view-model.js';
  ...
  return (
    <main className="app-shell">
      ...
      <section className="page-intro">
        <div>
          <span className="eyebrow">Personal sports intelligence</span>
          <h1>GameDay</h1>
          <p>Every game ranked by competitive value, stakes, form, narrative, and what matters to you.</p>
        </div>
        <div className="window-note">7-day radar</div>
      </section>
      ...
      {view === 'my-games'
        ? <MyGamesView games={filteredGames} now={today} onOpenGame={(game) => setSelectedGameId(game.identity.gameId)} />
        : <CalendarView games={filteredGames} cursor={cursor} onShiftWeek={shiftWeek} onOpenGame={(game) => setSelectedGameId(game.identity.gameId)} />}
      <GameDetailModal game={selectedGame} onClose={() => setSelectedGameId(null)} />
      <footer>
        <span>{loading ? 'Loading sports data…' : `${filteredGames.length} games in your 7-day view`}</span>

...
export function scoreGameV2(game) {
  ...
  const confidence = Number(((competitive.confidence * 25 + stakes.confidence * 25 + teamQuality.confidence * 15 + narrative.confidence * 10 + form.confidence * 10 + personal.confidence * 15) / 100).toFixed(2));

  return {
    total: Math.max(0, Math.min(100, total)),
    tier: getWatchTier(total).label,
    breakdown: { competitive, stakes, teamQuality, narrative, form, personal },
    confidence,
  };
}

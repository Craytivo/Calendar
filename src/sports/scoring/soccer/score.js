...
function minute(game) {
  ...
  return seconds === undefined ? undefined : Math.max(0, Math.min(120, seconds / 60));
}
...
function importance(game) {
  let value = Math.min(5, getPriorityScore(game) / 20);
...
function resultTension(game) {
  ...
  return Math.min(40, base * (0.55 + pressure * 0.65));
}
...
function soccerActivity(game) {
  const stats = game?.soccerStats;
  if (!stats?.home || !stats?.away) return { chancePressure: 0, intensity: 0, available: false };
  const min = minute(game);
  const elapsed = Math.max(15, Math.min(105, min ?? 45));
  const home = stats.home;
  const away = stats.away;
  const totalShots = finite(home.shots) !== undefined && finite(away.shots) !== undefined ? finite(home.shots) + finite(away.shots) : undefined;
  const totalOnTarget = finite(home.shotsOnTarget) !== undefined && finite(away.shotsOnTarget) !== undefined ? finite(home.shotsOnTarget) + finite(away.shotsOnTarget) : undefined;
  const shotRate = totalShots === undefined ? 0 : Math.min(1, totalShots / (elapsed * 0.28));
  const targetRate = totalOnTarget === undefined ? 0 : Math.min(1, totalOnTarget / (elapsed * 0.09));
  const chancePressure = Math.round(10 * (shotRate * 0.35 + targetRate * 0.65));
  const cards = (finite(home.yellowCards) ?? 0) + (finite(away.yellowCards) ?? 0) + ((finite(home.redCards) ?? 0) + (finite(away.redCards) ?? 0)) * 2;
  const cardIntensity = Math.min(5, cards * (elapsed >= 75 ? 1 : 0.5));
  const cornerPressure = finite(home.corners) !== undefined && finite(away.corners) !== undefined ? Math.min(3, (finite(home.corners) + finite(away.corners)) / 4) : 0;
  const intensity = Math.round(Math.min(8, cardIntensity + cornerPressure + chancePressure * 0.45));
  return { chancePressure, intensity, available: totalShots !== undefined || totalOnTarget !== undefined || cards > 0 || cornerPressure > 0 };
}
...
function expectedOutcomeSurprise(game) {
  ...
  const values = [home, draw, away].map(probability);
...
function liveRawScore(game) {
  const min = minute(game);
  const elapsed = min === undefined ? 0 : Math.min(1, min / 90);
  const pregameWeight = 0.35 * (1 - elapsed) + 0.05;
  const activity = soccerActivity(game);
  const observed = resultTension(game) + latePressure(game) + expectedOutcomeSurprise(game) + importance(game) + activity.chancePressure + activity.intensity;
  return observed + pregameInterest(game) * pregameWeight;
}

function finalRawScore(game) {
  const diff = margin(game);
  const total = goals(game);
  if (diff === undefined || total === undefined) return Math.min(20, pregameInterest(game) + importance(game));
  let excitement;
  if (diff === 0) excitement = total === 0 ? 8 : total === 2 ? 30 : total * 12;
  else if (diff === 1) excitement = 28 + Math.min(18, total * 3);
  else if (diff === 2) excitement = 14 + Math.min(12, total * 2);
  else excitement = Math.max(2, 8 - diff);
  ...
  return excitement + Math.min(10, importance(game) * 0.6);
}

function scheduledRawScore(game) {
  const context = market(game);
  const competitive = context.available ? context.contribution : 0;
  return pregameInterest(game) + importance(game) + competitive;
}

// Standardized 0–100 scale:
// 50 = objectively average, 0 = awful, 100 = exceptional / one-of-one.
// Each game state has its own raw-score distribution, but the displayed scale is shared.
function standardize(raw, { min, average, exceptional }) {
  if (!Number.isFinite(raw)) return 50;
  if (raw <= average) {
    if (raw <= min) return 0;
    return Math.round(((raw - min) / (average - min)) * 50);
  }
  if (raw >= exceptional) return 100;
  return Math.round(50 + ((raw - average) / (exceptional - average)) * 50);
}

function normalizeSoccerScore(game, raw) {
  if (game?.status === 'scheduled') return standardize(raw, { min: 0, average: 15, exceptional: 45 });
  if (game?.status === 'final') return standardize(raw, { min: 8, average: 25, exceptional: 50 });
  return standardize(raw, { min: 5, average: 45, exceptional: 82 });
}
...
export function getSoccerRawScore(game) {
  ...
  if (game?.status === 'live') return normalizeSoccerScore(game, liveRawScore(game));
  if (game?.status === 'final') return normalizeSoccerScore(game, finalRawScore(game));
  return normalizeSoccerScore(game, scheduledRawScore(game));
}
...
export function getSoccerScoreComponents(game) {
  ...
  return {
    ...
    chancePressure: activity.chancePressure,
    matchIntensity: activity.intensity,
    matchMinute: min,
...
export function getSoccerScoreReasons(game) {
  ...
  if (game?.status === 'live') {
    ...
    if (components.chancePressure >= 6) reasons.push('High chance volume');
    ...
    if (components.goals >= 5) reasons.push('High-scoring match');

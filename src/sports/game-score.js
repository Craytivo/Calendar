...
function totalPoints(game) {
  const away = scoreFor(game, 'away');
  const home = scoreFor(game, 'home');
  return away === undefined || home === undefined ? undefined : away + home;
}
...
function closeScoreBonus(game) {
  ...
  if (diff === 0) return 40;
  if (diff === 1) return SOCCER.has(game?.leagueId) ? 34 : 38;
  if (diff === 2) return 28;
  if (diff <= 4) return 20;
  if (diff <= 7) return 10;
  return 0;
...
function liveDrama(game) {
  ...
  let score = 12 + closeScoreBonus(game) + lateGameBonus(game);
...
function finalDrama(game) {
  ...
  if (SOCCER.has(sport)) {
    if (total !== undefined && total >= 5) score += 20;
    else if (total !== undefined && total >= 4) score += 12;
  } else if (FIELD_SPORTS.has(sport)) {
    if (total !== undefined && total >= 60) score += 18;
    else if (total !== undefined && total >= 45) score += 10;
  } else if (sport === 'mlb') {
    if (total !== undefined && total >= 10) score += 14;
    if (diff !== undefined && diff <= 2) score += 8;
  }

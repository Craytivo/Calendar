...
function normalizeEntry(entry, group) {
  ...
  const gamesPlayed = number(stat(entry, ['gamesPlayed', 'games'])) ??
    (wins !== undefined && losses !== undefined ? wins + losses + ties : undefined);
  const winPercentage = number(stat(entry, ['winPercent', 'winPct', 'winningPercentage', 'winpercentage'])) ??
    (wins !== undefined && gamesPlayed ? (wins + ties * 0.5) / gamesPlayed : undefined);
...
function getEntries(payload) {
  return (payload?.groups ?? []).flatMap((group) =>
    (group.entries ?? []).map((entry) => normalizeEntry(entry, group)),
  );
}
...
function isLateSeason(game) {
  ...
  return (
    (game.leagueId === 'nfl' && date.getMonth() >= 11) ||
...
function significantPlayoffRace(game) {
  ...
  if (!home || !away || !cutoff) return false;

  const ranks = [home.conferenceRank ?? home.leagueRank, away.conferenceRank ?? away.leagueRank]
...
function enrichGame(game, standingsByName) {
  ...
  const isDivisional = Boolean(
    homeTeam?.division && awayTeam?.division && homeTeam.division === awayTeam.division,
  );
  ...
  const enriched = {
    ...
    homeTeam,
    awayTeam,
    isDivisional: game.leagueId === 'nfl' ? isDivisional : game.isDivisional,
  };
...
export async function enrichGamesWithEspnStandings(games, year = new Date().getFullYear()) {
  const leagueIds = [...new Set(games.map((game) => game.leagueId))]
    .filter((leagueId) => ESPN_STANDINGS[leagueId]);

  const results = await Promise.allSettled(
    leagueIds.map(async (leagueId) => [leagueId, await fetchEspnStandings(leagueId, year)]),
  );

...
export function normalizeTeamContext(raw = {}) {
  const team = raw.team ?? raw; const context = normalizeTeam(team);
  ...
  const wins = toNumber(firstDefined(raw.wins,team.wins)); const losses = toNumber(firstDefined(raw.losses,team.losses)); const ties = toNumber(firstDefined(raw.ties,team.ties));
  const points = toNumber(firstDefined(raw.points,raw.leaguePoints,team.points,team.leaguePoints)); const goalDifference = toNumber(firstDefined(raw.goalDifference,raw.goalDiff,team.goalDifference,team.goalDiff));
  return { ...context, ...(winPercentage !== undefined ? {winPercentage}:{}), ...(gamesPlayed !== undefined ? {gamesPlayed}:{}), ...(conferenceRank !== undefined ? {conferenceRank}:{}), ...(leagueRank !== undefined ? {leagueRank}:{}), ...(ranking !== undefined ? {ranking}:{}), ...(wins !== undefined ? {wins}:{}), ...(losses !== undefined ? {losses}:{}), ...(ties !== undefined ? {ties}:{}), ...(points !== undefined ? {points}:{}), ...(goalDifference !== undefined ? {goalDifference}:{} ) };
}
export function normalizeGame(raw = {}) {
  const startTime = firstDefined(raw.startTime,raw.startDate,raw.date,raw.start); const status = normalizeStatus(firstDefined(raw.status,raw.gameStatus,raw.state)); const eventType = normalizeEventType(firstDefined(raw.eventType,raw.type,raw.phase));
  const homeRaw = firstDefined(raw.homeTeam,raw.home,raw.homeCompetitor); const awayRaw = firstDefined(raw.awayTeam,raw.away,raw.awayCompetitor);
  const homeTeam = homeRaw ? normalizeTeamContext(homeRaw) : undefined; const awayTeam = awayRaw ? normalizeTeamContext(awayRaw) : undefined;
...
export function isNormalizedGame(game) { return Boolean(game && game.id && game.startTime && VALID_STATUSES.has(game.status) && VALID_EVENT_TYPES.has(game.eventType)); }

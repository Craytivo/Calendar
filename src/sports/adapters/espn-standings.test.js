...
function stat(name, value) {
  return { name, value: String(value), displayValue: String(value) };
}
...
export async function runEspnStandingsChecks() {
  globalThis.fetch = async (url) => {
    ...
    if (text.includes('/basketball/nba/')) {
      return new Response(JSON.stringify({
        groups: [{
  ...
  const nfl = await enrichGamesWithEspnStandings([{
    id: 'nfl-test',
    leagueId: 'nfl',
    startTime: '2026-09-20T18:00:00.000Z',
    status: 'scheduled',
    eventType: 'regular-season',
    homeTeam: { id: 'tsdb:nfl:1', name: 'Kansas City Chiefs', leagueId: 'nfl' },
    awayTeam: { id: 'tsdb:nfl:2', name: 'Denver Broncos', leagueId: 'nfl' },
  }]);
  ...
  const nba = await enrichGamesWithEspnStandings([{
    id: 'nba-test',
    leagueId: 'nba',
    startTime: '2026-11-20T03:00:00.000Z',
    status: 'scheduled',
    eventType: 'regular-season',
    homeTeam: { id: 'sac-kings', name: 'Sacramento Kings', leagueId: 'nba' },
    awayTeam: { id: 'tsdb:nba:4', name: 'Denver Nuggets', leagueId: 'nba' },
  }]);
...
runEspnStandingsChecks().catch((error) => {
  globalThis.fetch = originalFetch;
  throw error;
});

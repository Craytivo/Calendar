...
import { getPriorityReasons, getPriorityTier } from '../sports/priority.js';
...
export function GameDetailModal({ game, onClose }) {
  ...
  const priority = scoreGameV2(game);
  const priorityScore = priority.total;
  const liveGameScore = getGameScoreSnapshot(game);
  const awayScore = scoreFor(game, 'away');
  const homeScore = scoreFor(game, 'home');
  const isLive = game.status === 'live';
  const isFinal = game.status === 'final';
  const metaScore = formatScoreboardMeta(game);
  ...
  return <div className="game-detail-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><div className={`game-detail-modal ${isLive ? 'is-live' : ''} ${isFinal ? 'is-final' : ''}`} role="dialog" aria-modal="true" aria-labelledby="game-detail-title">
    <div className="game-detail-hero"><header className="game-detail-header"><div className="game-detail-league"><span className="game-detail-eyebrow">{league?.shortName || game.leagueId.toUpperCase()}</span></div><button type="button" className="icon-button game-detail-close" onClick={onClose} aria-label="Close game details"><X size={18} /></button></header>
      <div className="game-detail-status-row"><span className={`game-detail-status ${isLive ? 'live' : ''}`}>{isLive ? <span className="game-detail-live-dot" /> : <span className="game-detail-status-mark" />}{statusLabel(game)}</span><span className="game-detail-watch"><strong>{priorityScore}</strong><span>{priority.tier}</span></span></div>
      ...
      <div className="game-detail-watchline"><div><span>Priority</span><strong>{priorityScore} · {priority.tier}</strong></div>{isLive && <div><span>Live Game Score</span><strong>{liveGameScore.score} · {liveGameScore.level}</strong></div>}{isLive && <div className="game-detail-watch-reasons">{liveGameScore.reasons.map((reason) => <span key={reason}>{reason}</span>)}</div>}</div>
      {importanceReasons.length > 0 && <div className="game-detail-storyline"><div><span>Why this matters</span><strong>{importanceSummary}</strong><div className="game-detail-reasons">{importanceReasons.map((reason) => <span key={reason}>{reason}</span>)}</div><small>Priority is the V9 pregame ranking. Live games additionally expose the live Game Score for current on-field excitement.</small></div></div>}
    </div>

...
function sourceRecord(id, name, provider, status, count = 0, error = null, durationMs = null, cached = false) {
  return { id, name, provider, status, count, ...(error ? { error } : {}), ...(Number.isFinite(durationMs) ? { durationMs } : {}), ...(cached ? { cached: true } : {}) };
}
...
async function cached(key, ttlSeconds, loader) {
  ...
  if (hit && hit.expiresAt > now) return { value: hit.value, cached: true, durationMs: 0 };
...
async function fetchSoccerTable(league, today) {
  ...
  return { table: result.data.table ?? [], durationMs: result.durationMs };
}
...
export default async function handler(req, res) {
  ...
  for (let index = 0; index < scheduleResults.length; index += 1) {
  ...
  favoriteResults.forEach((result, index) => {
    ...
    if (result.status === 'fulfilled') {
      ...
      diagnostics.favorites.push(sourceRecord(teamId, teamId, 'ESPN favorite-team fallback', 'error', 0, errorMessage(result.reason)));
    }
  });
  ...
  const tableResults = await Promise.allSettled(soccerLeagues.map(async (league) => [league.id, await cached(`soccer-standings:${league.id}:${seasonFor(league.id, today)}`, STANDINGS_CACHE_SECONDS, () => fetchSoccerTable(league, today).then((result) => result.table))]));
  ...
  const gamesNeedingStandings = enrichedGames.some((game) => STANDINGS_LEAGUES.has(game.leagueId));
  if (gamesNeedingStandings) {
    try {
      enrichedGames = await enrichGamesWithEspnStandings(enrichedGames, today.getUTCFullYear());

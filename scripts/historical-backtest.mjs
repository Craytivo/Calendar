...
function eventToGame(event, leagueId, status = 'final') {
  ...
  const historicalEndClock = ['epl', 'epl-cup', 'laliga', 'ucl'].includes(leagueId) ? 90 * 60 : leagueId === 'mlb' ? 9 : 0;
  ...
  return {
    ...
    period: status === 'final' ? (period ?? (leagueId === 'mlb' ? 9 : ['epl', 'epl-cup', 'laliga', 'ucl'].includes(leagueId) ? 2 : 4)) : period,
    clock,
    clockSeconds: status === 'final' ? (clockSeconds ?? historicalEndClock) : clockSeconds,
...
function normalizeOdds(odds) {
  ...
  return { spread, total, homeMoneyline: home, awayMoneyline: away, ...(draw !== undefined ? { drawMoneyline: draw } : {}) };
}
...
function quantile(sorted, q) {
  if (!sorted.length) return 0;
  const index = (sorted.length - 1) * q; const lower = Math.floor(index); const upper = Math.ceil(index);
...
function pearson(a, b) {
  ...
  const ax = pairs.reduce((s, p) => s + p[0], 0) / pairs.length; const by = pairs.reduce((s, p) => s + p[1], 0) / pairs.length;
...
function rank(values) {
  const indexed = values.map((value, index) => ({ value, index })).sort((a, b) => a.value - b.value); const ranks = Array(values.length);
  for (let i = 0; i < indexed.length;) { let j = i + 1; while (j < indexed.length && indexed[j].value === indexed[i].value) j += 1; const r = (i + j - 1) / 2 + 1; for (let k = i; k < j; k += 1) ranks[indexed[k].index] = r; i = j; }
  return ranks;
}
function spearman(a, b) { return pearson(rank(a), rank(b)); }

function objectiveExcitement(game, leagueValues) {
  const diff = Math.abs(Number(game.homeScore) - Number(game.awayScore)); const total = Number(game.homeScore) + Number(game.awayScore);
  const typicalDiff = Math.max(1, quantile(leagueValues.diffs, 0.75)); const typicalTotal = Math.max(1, quantile(leagueValues.totals, 0.75));
  const closeness = clamp(1 - diff / (typicalDiff * 1.8)); const scoring = clamp(total / (typicalTotal * 1.5));
  const overtime = game.isOvertime ? 1 : 0; const postseason = game.eventType !== 'regular-season' ? 1 : 0;
  return Math.round(100 * (0.55 * closeness + 0.20 * scoring + 0.15 * overtime + 0.10 * postseason));
}

function buildCalibration(rawScores) {
  const sorted = [...rawScores].sort((a, b) => a - b); const percentiles = [0.01, 0.05, 0.10, 0.20, 0.25, 0.50, 0.75, 0.90, 0.95, 0.99];
  return { sampleSize: sorted.length, rawQuantiles: Object.fromEntries(percentiles.map((q) => [`p${String(q * 100).padStart(2, '0')}`, Math.round(quantile(sorted, q) * 100) / 100])), ecdfAnchors: Array.from({ length: 101 }, (_, raw) => ({ raw, calibrated: Math.round(percentile(sorted, raw) * 100) })) };
}

function thresholdRates(scores) { const n = scores.length || 1; return Object.fromEntries([60, 70, 80, 90, 95].map((threshold) => [threshold, Math.round((scores.filter((s) => s >= threshold).length / n) * 10000) / 100])); }
...
function analyzeLeague(games, leagueId) {
  const values = { diffs: games.map((g) => Math.abs(g.homeScore - g.awayScore)).sort((a, b) => a - b), totals: games.map((g) => g.homeScore + g.awayScore).sort((a, b) => a - b) };
  const rows = games.map((finalGame) => {
    ...
    const finalRaw = getRawGameScore(finalGame); const pregameRaw = getRawGameScore(pregame); const signals = getGameScoreSignals(finalGame); const target = objectiveExcitement(finalGame, values);
  ...
  const pregameScores = rows.map((r) => r.pregameRaw); const finalScores = rows.map((r) => r.finalRaw); const target = rows.map((r) => r.target);
  const signalNames = ['favorite', 'priority', 'closeScore', 'lateGame', 'overtime', 'elimination', 'implications', 'baseInterest', 'marketExcitement']; const correlations = {};
  for (const signal of signalNames) { const valuesForSignal = rows.map((r) => Number(r.signals[signal] ?? 0)); correlations[signal] = { targetPearson: Number(pearson(valuesForSignal, target).toFixed(4)), targetSpearman: Number(spearman(valuesForSignal, target).toFixed(4)), scorePearson: Number(pearson(valuesForSignal, pregameScores).toFixed(4)) }; }
  const redundancy = [];
  for (let i = 0; i < signalNames.length; i += 1) for (let j = i + 1; j < signalNames.length; j += 1) { const a = rows.map((r) => Number(r.signals[signalNames[i]] ?? 0)); const b = rows.map((r) => Number(r.signals[signalNames[j]] ?? 0)); const correlation = pearson(a, b); if (Math.abs(correlation) >= 0.65) redundancy.push({ a: signalNames[i], b: signalNames[j], correlation: Number(correlation.toFixed(4)) }); }
  redundancy.sort((a, b) => Math.abs(b.correlation) - Math.abs(a.correlation));
  return {
    leagueId, sampleSize: rows.length,
    raw: {
    ...
    signalPredictiveness: correlations, redundantSignals: redundancy,
  };
}
...
function markdown(report) {
  ...
  for (const result of Object.values(report.leagues)) { const t = result.raw.pregame.thresholds; lines.push(`| ${result.leagueId} | ${result.sampleSize} | ${result.raw.pregame.mean} | ${result.raw.final.mean} | ${result.raw.inflation.meanDelta.toFixed(2)} | ${t[60]}% | ${t[70]}% | ${t[80]}% | ${t[90]}% | ${t[95]}% | ${result.target.pregameSpearman.toFixed(3)} |`); }
  lines.push('', '## Interpretation rules', '', '- Inflation = final raw score minus pregame raw score.', '- Redundancy flags signal pairs with absolute Pearson correlation ≥ 0.65.', '- Predictive value uses Spearman correlation between pregame raw score and an outcome-only historical excitement target. This is a ranking diagnostic, not a causal claim.', '- Calibration curves are empirical ECDF mappings from raw score → percentile score.', '- Do not replace the production calibration curve automatically; review sample size and out-of-sample stability first.');
  return `${lines.join('\n')}\n`;
}
...
async function main() {
  ...
  for (const [leagueId, config] of Object.entries(LEAGUES)) {
    process.stdout.write(`\n[${leagueId}] collecting ${START_YEAR}-${END_YEAR}...`); const result = await collectLeague(leagueId, config); process.stdout.write(` ${result.games.length} games\n`);
    if (result.errors.length) report.errors.push(...result.errors.map((error) => ({ leagueId, ...error })));
    report.leagues[leagueId] = result.games.length ? analyzeLeague(result.games, leagueId) : { leagueId, sampleSize: 0, errors: result.errors };
